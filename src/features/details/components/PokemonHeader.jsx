...
const PokemonHeader = ({ pokemon, getTypeColor }) => (
  <div className="bg-[#1E1E1E] rounded-2xl shadow-2xl overflow-hidden mb-10 backdrop-blur-xl">
    <div 
      ...
      style={{
        background: `radial-gradient(circle at 50% 0%, ${getTypeColor(pokemon.types[0])}33 0%, transparent 75%), 
                    linear-gradient(to bottom, ${getTypeColor(pokemon.types[0])}22, transparent)`,
      ...
      <div className="flex flex-col md:flex-row items-center">
        <div className="w-full md:w-2/5 mb-8 md:mb-0 flex justify-center">
          <div className="relative">
            <div className="absolute inset-0 bg-white opacity-5 blur-2xl rounded-full scale-90" 
              style={{ background: `radial-gradient(circle, ${getTypeColor(pokemon.types[0])}66, transparent 70%)` }}>
            </div>
            <img 
              src={pokemon.image} 
              alt={pokemon.name} 
        ...
        <div className="w-full md:w-3/5 md:pl-8">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="bg-[#00000030] backdrop-blur-sm rounded-xl p-3 text-center">
              <p className="text-xs text-gray-400 uppercase tracking-wide">Habitat</p>
              <p className="font-semibold text-lg capitalize">{pokemon.species.habitat}</p>
            </div>
          </div>
        </div>
      </div>
